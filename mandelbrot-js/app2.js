...
(function () {
  ...
  Keys.key("Escape", [], "Escape to reset", null, e => {
    c0 = c1 = c2 = undefined;
    stepsValuesIdx = 0;
    steps = stepsValues[stepsValuesIdx];
    init();
  });
  Keys.key("Space", [], "TBD",
    e => {
  ...
  Keys.key("KeyF", [], "Save to file", e => {
    ...
    // A4: 210mm × 297mm
    const sideSize = 29.7 / 2.54; // inches
    const pixelsPerInch = 600;
    const imageSize = Math.max(ctx.canvas.width, ctx.canvas.height);
    const f = (sideSize * pixelsPerInch) / imageSize;
    canvas.width = Math.round(ctx.canvas.width * f);
