...
export class Workerp {
  ...
  _init() {
    this.worker = new Worker(this.scriptSource, {type: "module"});
    this.promises = {};
    ...
    this.worker.onmessage = function (e) {
      ...
      if (!callId) {
        throw new Error("Worker [" + self.scriptSource + "] must return callId as the part of event.data");
      }

      if (!self.promises[callId]) {
        throw new Error("Worker [" + self.scriptSource + "] returned unknown callId " + callId);
      }
